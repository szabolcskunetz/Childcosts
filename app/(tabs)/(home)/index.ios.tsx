...
export default function ExpensesScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (participantsData.length > 0 && !newExpense.paidBy) {
        setNewExpense(prev => ({
          ...prev,
  ...
  const handleExportSelected = async () => {
    try {
      ...
      console.log('[Export] Starting export selected:', selectedExpenseIds.size, 'expenses');
      
      const selectedExpenses = sortedExpenses.filter(e => selectedExpenseIds.has(e.id));
      
      const header = 'Date,Description,Amount,Paid By,Split Percentage,Created By\n';
      ...
      const rows = selectedExpenses.map(expense => {
        ...
        const createdById = typeof expense.createdBy === 'string' 
          ? expense.createdBy 
          : (expense.createdBy as any)?.id || 'Unknown';
        ...
        return `${dateStr},"${expense.description}",${expense.amount},${paidByName},${expense.splitPercentage},${createdByName}`;
      }).join('\n');
      
      const csv = header + rows;
      const fileName = `expenses_selected_${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const handleImport = async () => {
    try {
      ...
      const errorsText = importResult.errors.length > 0 ? `\n\n${t('importErrors')}: ${importResult.errors.join(', ')}` : '';
                  ...
                  <View style={styles.expenseHeader}>
                    ...
                    {canModify && !selectionMode && (
                      <View style={styles.actionButtons}>
                        <TouchableOpacity
                          onPress={() => {
                            console.log('[Expense] Edit button tapped for expense:', expense.id);
                        ...
                        <TouchableOpacity
                          onPress={() => {
                            console.log('[Expense] Delete button tapped for expense:', expense.id);
        ...
        >
          ...
          <View style={styles.exportOptionText}>
            ...
            <Text style={[styles.exportOptionDescription, { color: theme.dark ? '#98989D' : '#666' }]}>
              {t('exportAllDescription') || 'Export all expenses to Excel file'}
            </Text>
          </View>
        </TouchableOpacity>
