...
export function registerExpensesRoutes(app: App) {
  ...
  app.fastify.get<{
    ...
    try {
      ...
      app.logger.info({ count: enrichedExpenses.length }, 'Expenses fetched successfully');
      return enrichedExpenses;
    } catch (error) {
      app.logger.error({ err: error, search, minAmount, maxAmount }, 'Failed to fetch expenses');
      throw error;
    }
  });
  ...
  app.fastify.post<{
    ...
    try {
      ...
      app.logger.info({ expenseId: newExpense[0].id, createdBy }, 'Expense created successfully');
      return result;
    } catch (error) {
      app.logger.error({ err: error, body: request.body }, 'Failed to create expense');
      throw error;
    }
  });
  ...
  app.fastify.put<{
    ...
    try {
      ...
      if (!session) {
        app.logger.warn({ expenseId: id }, 'Unauthenticated attempt to update expense');
        return;
      }

      // Check if expense exists and createdBy matches
      const expense = await app.db.select().from(expenses).where(eq(expenses.id, id));
      ...
      if (expense[0].createdBy !== null && expense[0].createdBy !== session.user.id) {
        app.logger.warn({ expenseId: id, userId: session.user.id, createdBy: expense[0].createdBy }, 'User attempted to update expense they do not own');
        reply.code(403);
        return { error: 'Only the creator can modify this expense' };
      }

      const { date, description, amount, paidBy, splitPercentage } = request.body;
      ...
      if (splitPercentage) updateData.splitPercentage = splitPercentage.toString();
      ...
      const result = {
        ...
        paidBy: paidByParticipant[0]
          ...
          : null,
        splitPercentage: parseFloat(updatedExpense[0].splitPercentage as string),
      ...
      app.logger.error({ err: error, expenseId: id, body: request.body }, 'Failed to update expense');
      throw error;
    }
  });
  ...
  app.fastify.delete<{
    ...
    try {
      ...
      const session = await app.requireAuth()(request, reply);
      if (!session) {
        app.logger.warn({ expenseId: id }, 'Unauthenticated attempt to delete expense');
        return;
      }
      ...
      // 2. createdBy matches the authenticated user ID
      if (expense[0].createdBy !== null && expense[0].createdBy !== session.user.id) {
        app.logger.warn({ expenseId: id, userId: session.user.id, createdBy: expense[0].createdBy }, 'User attempted to delete expense they do not own');
        reply.code(403);
      ...
      app.logger.info({ expenseId: id, userId: session.user.id }, 'Expense deleted successfully');
      return { success: true };
    } catch (error) {
      app.logger.error({ err: error, expenseId: id }, 'Failed to delete expense');
      throw error;
    }
  });
  ...
  app.fastify.get<{
    ...
    try {
      ...
      // Build data rows
      const headers = ['Date', 'Description', ...allParticipants.map((p) => `Paid by ${p.name}`)];
      const rows = [headers];
      ...
      for (const expense of allExpenses) {
        const row = [
          ...
          expense.description,
          ...allParticipants.map((p) => (p.id === expense.paidBy ? expense.amount : '0')),
        ];
        rows.push(row);
      }
      ...
      if (format === 'xlsx') {
        ...
        reply.header('Content-Disposition', 'attachment; filename="expenses.xlsx"');
        app.logger.info({ expenseCount: allExpenses.length, format }, 'Expenses exported successfully');
        return excelBuffer;
      } else {
      ...
      app.logger.error({ err: error, ids, format }, 'Failed to export expenses');
      throw error;
    }
  });
  ...
  app.fastify.post<{ Querystring: { projectId?: string } }>('/api/expenses/import', async (request, reply) => {
    ...
    try {
      ...
      try {
        ...
        app.logger.warn({ err: fileError }, 'Error retrieving file from request');
        data = null;
      }
      ...
      app.logger.info({ filename, encoding: data.encoding, mimetype: data.mimetype }, 'Processing file upload');
      ...
      if (!isExcel && !isCSV) {
        ...
        reply.code(400);
        return { error: 'Invalid file format. Please upload a CSV or Excel file (.csv, .xlsx, .xls)', imported: 0, errors: [] };
      }
      ...
      try {
        if (isExcel) {
          ...
          try {
            ...
            app.logger.info({ sheets: workbook.SheetNames }, 'Excel workbook loaded');

            const worksheet = workbook.Sheets[workbook.SheetNames[0]];
            if (!worksheet) {
              reply.code(400);
              return { error: 'Excel file has no sheets', imported: 0, errors: [] };
            }
            ...
            if (data.length === 0) {
              reply.code(400);
              return { error: 'Excel file is empty', imported: 0, errors: [] };
            }

            headers = data[0] || [];
            app.logger.info({ headerCount: headers.length, rowCount: data.length - 1 }, 'Excel file parsed');
            ...
            app.logger.error({ err: excelError }, 'Failed to parse Excel file');
            reply.code(400);
            return { error: `Failed to parse Excel file: ${excelError instanceof Error ? excelError.message : 'Unknown error'}`, imported: 0, errors: [] };
          }
        } else {
          // Parse CSV file
          try {
            ...
            app.logger.info({ headerCount: headers.length, rowCount: records.length }, 'CSV file parsed');
          } catch (csvError) {
            app.logger.error({ err: csvError }, 'Failed to parse CSV file');
            reply.code(400);
            return { error: `Failed to parse CSV file: ${csvError instanceof Error ? csvError.message : 'Unknown error'}`, imported: 0, errors: [] };
          }
        }
      } catch (parseError) {
        app.logger.error({ err: parseError }, 'Unexpected error during file parsing');
        throw parseError;
      }
      ...
      if (!firstParticipant) {
        reply.code(400);
        return { error: 'No participants available', imported: 0, errors: [] };
      }
      const createdById = firstParticipant.id;

      // Helper function to find column by various possible names (multilingual support)
      const findColumn = (record: Record<string, any>, possibleNames: string[]): [string | null, any] => {
      ...
      const findParticipantColumns = (): Map<string, { headerName: string; pattern: string }> => {
        const participantColumns = new Map<string, { headerName: string; pattern: string }>();
        ...
        for (const header of headers) {
          ...
          for (const pattern of patterns) {
            ...
            if (match) {
              ...
              participantColumns.set(header, { headerName: header, pattern: pattern.source });
              break;
            }
          }
        }
      ...
      const parseDate = (dateValue: any): { date: Date | null; originalValue: string } => {
        ...
        if (!isNaN(numValue) && numValue > 0 && numValue < 100000) {
          ...
          const excelDate = new Date(excelEpochTime + numValue * 24 * 60 * 60 * 1000);

          // Validate the date is in a reasonable range (1900-2100)
          if (!isNaN(excelDate.getTime()) && excelDate.getFullYear() >= 1900 && excelDate.getFullYear() <= 2100) {
            app.logger.debug({ serial: numValue, parsed: excelDate.toISOString() }, 'Excel serial date parsed');
        ...
        const datePatterns = [
          { regex: /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/, parse: (m: string[]) => new Date(parseInt(m[3]), parseInt(m[1]) - 1, parseInt(m[2])) }, // MM/DD/YYYY
          { regex: /^(\d{1,2})\.(\d{1,2})\.(\d{4})$/, parse: (m: string[]) => new Date(parseInt(m[3]), parseInt(m[2]) - 1, parseInt(m[1])) }, // DD.MM.YYYY
          { regex: /^(\d{4})\.(\d{1,2})\.(\d{1,2})$/, parse: (m: string[]) => new Date(parseInt(m[1]), parseInt(m[2]) - 1, parseInt(m[3])) }, // YYYY.MM.DD
          { regex: /^(\d{4})-(\d{1,2})-(\d{1,2})$/, parse: (m: string[]) => new Date(parseInt(m[1]), parseInt(m[2]) - 1, parseInt(m[3])) }, // YYYY-MM-DD
        ];
      ...
      for (let i = 0; i < records.length; i++) {
        try {
          ...
          const allEmpty = Object.values(record).every((v) => !v || String(v).trim() === '');
          if (allEmpty) {
          ...
          if (!dateHeader) {
            errors.push(`Row ${i + 2}: Date column not found. Available columns: ${headers.join(', ')}`);
            continue;
          }
          ...
          if (!descHeader) {
            errors.push(`Row ${i + 2}: Description column not found. Available columns: ${headers.join(', ')}`);
            continue;
          }
          ...
          if (!date) {
            errors.push(`Row ${i + 2}: Invalid date '${dateOriginal}' in '${dateHeader}' column. Try ISO 8601 (2024-01-15), Excel date, or common formats (01/15/2024, 15.01.2024, 2024.01.15)`);
            continue;
          }

          // Find all participants with non-zero amounts in this row
          const participantsWithAmounts: Array<{ participantId: string; participantName: string; amount: number }> = [];
          ...
          for (const [header, colInfo] of participantCols) {
            ...
            if (!isNaN(amount) && amount > 0) {
              ...
              if (!participantId) {
                participantErrors.push(`Row ${i + 2}: Participant "${participantName}" not found in database. Available participants: ${Array.from(participantMap.keys()).join(', ')}`);
                continue;
              }
          ...
          if (participantsWithAmounts.length === 0) {
            errors.push(`Row ${i + 2}: No participant with non-zero amount found. Please check the 'Paid by [Name]' columns.`);
            continue;
          }

          // Create a separate expense for each participant who paid
          for (const { participantId, participantName, amount } of participantsWithAmounts) {
            try {
              ...
              app.logger.debug({ expenseId: expense[0].id, participant: participantName, amount }, 'Expense created from import');
            } catch (expenseError) {
              errors.push(`Row ${i + 2} (${participantName}): ${expenseError instanceof Error ? expenseError.message : 'Failed to create expense'}`);
            }
          }
        } catch (error) {
      ...
      app.logger.info({ imported: imported.length, errors: errors.length, format: isExcel ? 'xlsx' : 'csv' }, 'Expenses imported');
      return {
